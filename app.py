...
# -------------------------------
genai.configure(api_key="AIzaSyAUd8_UuRowt-QmJBESIBTEXC8dnSDWk_Y")  # directly insert API key
...
# -------------------------------
# Generate AI questions with fallback
# -------------------------------
...
# -------------------------------
# Determine app mode from query param or sidebar
# -------------------------------
...
if mode == "Host":
    ...
    st.subheader("📱 Players scan the QR code or click link to join:")

    # QR Code
    qr = qrcode.QRCode(version=1, box_size=8, border=2)
    qr.add_data(GAME_URL + "?mode=Player")
    qr.make(fit=True)
    img = qr.make_image(fill='black', back_color='white')
    buf = BytesIO()
    img.save(buf)
    st.image(buf, width=200)
    st.write(f"Or click here to join as Player: [Join Game]({GAME_URL}?mode=Player)")
